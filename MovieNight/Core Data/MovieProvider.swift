...
final class MovieProvider {
    ...
    static var preview: MovieProvider = {
        ...
        for index in 1..<10 {
            let movie_CD = Details_CD(context: viewContext)
    ...
    static var previewMovie: Details_CD = {
        let controller = MovieProvider(inMemory: true)
        let viewContext = controller.viewContext
    ...
    func exists(id: Int64) -> Details_CD? {
        let fetchRequest: NSFetchRequest<Details_CD> = Details_CD.all()
    ...
    func update(entity: Details_CD, userRating: Int16) {
        entity.userRating = userRating

        self.save()
    }

    func fetch() -> [Details_CD] {
        var results: [Details_CD] = []

        do {
            results = try viewContext.fetch(Details_CD.all())
        } catch {
